...
import com.ckevin.climaxapp.config.KafkaProperties;
...
class ClimaxAppKafkaResourceIT {
    ...
    void setup() {
        ...
        ClimaxAppKafkaResource kafkaResource = new ClimaxAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/climax-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/climax-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
